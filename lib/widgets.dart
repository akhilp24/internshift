                  ...
                  if (!snapshot.hasData) {
                    ...
                    child: GridView.builder(
                      ...
                      itemBuilder: (BuildContext context, int index) {
                        DocumentSnapshot bsnapshot = snapshot.data.documents[index];
                        return Card(
                          child: Container(
                          child: Center(child: Text(bsnapshot.data["nameofoffer"])),
                          width: 100
                          ), 
                          elevation: 2,);
                      },
...
void appAddModalBottomSheet(context, String nameofJob, String employer, String employerLocation, String logo) {
  showModalBottomSheet(context: context, backgroundColor: Colors.transparent, isScrollControlled: true, builder: (BuildContext bc) {
            ...
            children: [
              Padding(
                ...
                child: Align(child: GestureDetector(
                  ...
                  child: Icon(Icons.close, size: 25,)), alignment: Alignment.topRight,),
              ),
              Padding(
              ...
              Padding(
                padding: const EdgeInsets.only(top: 20.0),
                child: Text(nameofJob, style: TextStyle(fontSize: 30, fontWeight: FontWeight.w800), textAlign: TextAlign.center,),
              ),
              Padding(
                padding: const EdgeInsets.only(top: 8.0),
                child: Text(employer, style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600),),
              ),
              Padding(
                padding: const EdgeInsets.only(top: 8.0),
                child: Row(
                  ...
                  children: [
                    ...
                    SizedBox(width: 5),
                    Text(employerLocation, style: TextStyle(fontWeight: FontWeight.w300),)
                  ]
                ),
              ),
              Padding(
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    Align(alignment: Alignment.topCenter, child: Text("The text of the description", ))
                    ]
                  )
                ),
              ),
...
class ApplyandSavedButtons extends StatefulWidget {
  ...
  final String logo;
  ApplyandSavedButtons({this.nameOfJob, this.employer, this.employerLocation, this.logo});
  @override
                            ...
                            child: StreamBuilder(
                              ...
                              builder: (context, snapshot) {
                                ...
                                if (!snapshot.hasData) {
                                  ...
                                  String testingIfTrueName = snapshot.data.documents[0]['nameOfJob'];
                                String testingIfTrueEmployer = snapshot.data.documents[0]['employer'];
                                  if ((testingIfTrueName == widget.nameOfJob) && (testingIfTrueEmployer == widget.employer)) {
                                    return Icon(Icons.bookmark, color: Colors.amber, size: 30);
                                  } else {
                                    return Icon(Icons.bookmark_outline, color: Colors.amber, size: 30);
                                  }
                                }
                              }
                            )
                            // isSaved ? Icon(Icons.bookmark, color: Colors.amber, size: 30) : Icon(Icons.bookmark_outline, color: Colors.amber, size: 30),
                        ),
                          ),
                      );
                        }
